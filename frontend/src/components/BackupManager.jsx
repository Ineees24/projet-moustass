...
export default function AdminUsers() {
  ...
  const createBackup = async (type) => {
    ...
    try {
      ...
      alert(`✅ ${response.data.message}\nTaille: ${response.data.file_size || 'N/A'}`);
    } catch (err) {
      const errorMsg = err.response?.data?.message || `Erreur lors du backup ${type}`;
      setError(errorMsg);
      alert(`❌ ${errorMsg}`);
    } finally {
    ...
    <div className="admin-users">
      <header className="admin-users__header">
        ...
        <div className="admin-users__meta">
          ...
          {/* BOUTONS DE BACKUP AJOUTÉS ICI */}
          <button
            className="admin-users__btn admin-users__btn--backup"
            type="button"
            onClick={() => createBackup('full')}
            disabled={backupLoading}
            title="Créer un backup complet de la base de données"
          >
            {backupLoading ? '⏳' : '🗄️'} Backup Complet
          </button>
          
          <button
            className="admin-users__btn admin-users__btn--backup-incremental"
            type="button"
            onClick={() => createBackup('incremental')}
            disabled={backupLoading}
            title="Créer un backup incrémental (changements uniquement)"
          >
            {backupLoading ? '⏳' : '📦'} Backup Incrémental
          </button>
